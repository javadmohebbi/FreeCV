...
@section('content')
    ...
    <div class="content-wrapper">
        ...
        <!-- KB Content -->
        @include('templates.blogs.modules.kb')
...
@section('pageScripts')
    ...
    <script type="text/javascript">
        ...
        /** Load more comments */
        function loadMoreCommentsOnPageScroll(page){
            var articleId = '{{ $article->id }}';
            $.ajax(
                {
                    url: baseURL + '/comment/ajax/load/more/'+articleId+'?page=' + page,
                    type: 'get',
                    //dataType: 'html',
                    beforSend: function(){
                        $("#commentLoading").removeClass('hidden');
                        $("#commentLoading .error").addClass('hidden');
                        $("#commentLoading .text-info").removeClass('hidden');
                    }
                })
                .done(function (data){
                    var html = '';
                ...
                .fail(function(jqXHR, ajaxOptions, thrownError){
                    commentsPage--;
                    $("#commentLoading .text-info").addClass('hidden');
                    $("#commentLoading .error").removeClass('hidden');
                });
        }


        /** Comments Try Again */
        $("#btnCommentLoadTryAgain").click(function () {
            $("#commentLoading .error").addClass('hidden');
            $("#commentLoading .text-info").removeClass('hidden');
            loadMoreCommentsOnPageScroll(commentsPage++);
        });
        ...
        $("#btn-save-comment").click(function(){
            $("#btn-save-comment-stat").html(' ');
            $("#comment-insert-loading").addClass('hidden');
            $("#btn-save-comment-stat").addClass('hidden');
            var commenterName = '';
            var commenterEmail = '';
            var commenterComment = $("#comment-comment").val();
            @if(!Auth::check())
                commenterName = $("#comment-name").val();
                commenterEmail = $("#comment-email").val();
            @endif
            var blogId = $(this).attr('data-id');
            var url = baseURL + '/comment/insert/' + blogId;
            $("#btn-save-comment").addClass('hidden');
            $("#comment-insert-loading").removeClass('hidden');
            $("#btn-save-comment-stat").removeClass('hidden');
            $.ajax({
                url: url,
                type: 'post',
                data: {
                    ...
                    $("#btn-save-comment").removeClass('hidden');
                    $("#comment-insert-loading").addClass('hidden');

                    $("#btn-save-comment-stat").removeClass('text-danger');
                    $("#btn-save-comment-stat").addClass('text-success');

                    $("#comment-comment").val(' ');
                    $("#btn-save-comment-stat").html(data['msg']);

                }, error: function(data) {
                    var errors = data.responseJSON;
                    $("#btn-save-comment-stat").removeClass('text-success');
                    $("#btn-save-comment-stat").addClass('text-danger');
                    $("#btn-save-comment-stat").html('<ul style="list-style: circle"></ul>');
                    $.each(errors, function (key, val) {
                        $("#btn-save-comment-stat ul").append("<li>" + val + "</li>");
                    });
                    $("#btn-save-comment").removeClass('hidden');
                    $("#comment-insert-loading").addClass('hidden');
                }
